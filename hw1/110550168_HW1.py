...
class LinearRegression:
    ...
    def closed_form_fit(self, X, y):
        ...
        X_5col = np.c_[np.ones(X.shape[0]),X]
        b = np.dot(np.linalg.inv(np.dot(X_5col.T,X_5col)), np.dot(X_5col.T, y))
        self.closed_form_weights = b[1:]
        self.closed_form_intercept = b[0]
    ...
    def gradient_descent_fit(self, X, y, lr, epochs):
        ...
        cal_matrix = np.arange(5.)
        N = X.shape[0]
        X_5col = np.c_[np.ones(N),X]
        loss_table = []
        for i in range(epochs):
            loss = y - np.dot(X_5col,cal_matrix.T)
            pred = loss * (-1/N)
            grad = np.dot(X_5col.T,pred) 
            cal_matrix += (-lr) * grad.T
            if i % 1 == 0:
                loss_table.append(np.mean(np.power(loss,2)))
                # print("i: ",i,"  loss: ",np.mean(np.power(loss,2)))
        self.gradient_descent_weights = cal_matrix[1:]
        self.gradient_descent_intercept = cal_matrix[0]
        fig = plt.figure()
        plt.plot(np.arange(0,epochs,1),loss_table)
        plt.show()
    ...
    def get_mse_loss(self, prediction, ground_truth):
        # Return the value.
        return np.mean(np.power(ground_truth - prediction,2))
    ...
    def closed_form_predict(self, X):
        ...
        return np.dot(self.closed_form_weights ,X.T) + self.closed_form_intercept
    ...
    def gradient_descent_predict(self, X):
        # Return the prediction.
        return np.dot(self.gradient_descent_weights ,X.T) + self.gradient_descent_intercept
