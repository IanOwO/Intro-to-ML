...
from data_py.dataset import build_loader
...
def main(args, tlogger):
    ...
    for epoch in range(start_epoch, args.max_epochs):
        ...
        if train_loader is not None:
            ...
            cur_loss = train(args, epoch, model, scaler, amp_context, optimizer, schedule, train_loader)
            loss_arr.append(cur_loss.item())
            tlogger.print()
        else:
    ...
    plt.subplot(1,2,1)
    plt.plot(np.arange(0,args.max_epochs,1),loss_arr)
    plt.title("loss")
    plt.xlabel("epoch")
    plt.ylabel("loss")

    # plt.subplot(1,2,2)
    # plt.plot(np.arange(0,args.max_epochs,1),acc_arr)
    # plt.title("accurancy")
    # plt.xlabel("epoch")
    # plt.ylabel("acc")
